...
import { Ruler, Thermometer, Car, ArrowLeft, Filter } from "lucide-react"
import { Link, useParams } from "react-router-dom"
import { getDemoFacility, getDemoUnits, isDemoProvider } from "@/services/demoData"
import { formatCurrencyMonthly } from "@/lib/currency"

export default function DemoUnitBrowser() {
  const { providerId } = useParams()
  const [priceRange, setPriceRange] = useState("all")
  const [unitType, setUnitType] = useState("all")
  const [searchTerm, setSearchTerm] = useState("")

  // Check if this is a demo provider, otherwise redirect
  ...
  const units = getDemoUnits(providerId)
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="bg-gradient-primary text-primary-foreground py-2 px-4 text-center">
        <p className="text-sm font-medium">
          🎭 Demo Mode - This is a demonstration of the storage unit browsing experience
        </p>
      </div>
          ...
          <div className="lg:col-span-3">
            <div className="mb-6">
              <p className="text-muted-foreground">
                Showing {filteredUnits.length} available demo units
              </p>
            </div>
