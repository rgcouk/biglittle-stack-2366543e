-- Create demo facility data based on Total Storage Boston (without dependencies on demo users for now)
-- We'll create the demo users through the signup process instead

-- Insert demo facility based on Total Storage Boston
-- Using a temporary provider_id that we'll update after demo users are created
